...
public class AnswerTypeContextGenerator {
  ...
  public AnswerTypeContextGenerator(File dictDir) {
    try {
      ...
      System.out.println("dictDir.getAbsolutePath()" +dictDir.getAbsolutePath());
    }
    catch (IOException e) {
  ...
  private Set<String> getSynsetSet(Parse np) {
    ...
    System.out.println("Lemmas are" +Arrays.asList(lemmas));
    
    for (int li = 0; li < lemmas.length; li++) {  
    	System.out.println(lemmas[li].toCharArray());
      String[] synsets = wordnet.getParentSenseKeys(lemmas[li],"NN",0);
      System.out.println(Arrays.asList(synsets));
